...
SELECT payroll_year, AVG(prumerna_mzda) AS prumerna_mzda_all, potravina, AVG(prumerna_cena) AS prumerna_cena_all, AVG(prumerna_mzda) / AVG(prumerna_cena) as kolik_lze_nakoupit
		FROM t_beata_davidova_project_SQL_primary_final
		WHERE potravina LIKE "Chléb%" AND (payroll_year = "2006" OR payroll_year = "2018")
...
SELECT payroll_year, AVG(prumerna_mzda) AS prumerna_mzda_all, potravina, AVG(prumerna_cena) AS prumerna_cena_all, AVG(prumerna_mzda) / AVG(prumerna_cena) as kolik_lze_nakoupit
		FROM t_beata_davidova_project_SQL_primary_final
		WHERE potravina LIKE "Mléko%" AND (payroll_year = "2006" OR payroll_year = "2018")
		GROUP BY payroll_year
		; 

-- 3. Která kategorie potravin zdražuje nejpomaleji (je u ní nejnižší percentuální meziroční nárůst)
-- Nejpomaleji zdražilo mezi lety 2006 a 2018 máslo.

SELECT act.category_code, act.potravina, (act.prumerna_cena/prev.prumerna_cena)/(2018 - 2006 + 1) * 100 AS rocni_narust
	FROM (
		SELECT a.payroll_year, a.category_code, a.potravina, AVG(a.prumerna_cena) AS prumerna_cena
		FROM t_beata_davidova_project_SQL_primary_final a
...
LEFT JOIN (SELECT act.payroll_year, (act.prumerna_cena - prev.prumerna_cena)/prev.prumerna_cena AS rozdil_cena,
	...
	LEFT JOIN (
		...
		FROM t_beata_davidova_project_SQL_primary_final b
		GROUP BY b.payroll_year) prev ON act.payroll_year = prev.payroll_year+1
	) res2 ON res.year = res2.payroll_year -- růst cen a mezd ve stejném roce
LEFT JOIN (SELECT act.payroll_year, (act.prumerna_cena - prev.prumerna_cena)/prev.prumerna_cena AS rozdil_cena,
		(act.prumerna_mzda - prev.prumerna_mzda)/prev.prumerna_mzda AS rozdil_mzda
	FROM (
		SELECT a.payroll_year, AVG(a.prumerna_cena) AS prumerna_cena, AVG(a.prumerna_mzda) AS prumerna_mzda
	...
	LEFT JOIN (
		...
		FROM t_beata_davidova_project_SQL_primary_final b
		GROUP BY b.payroll_year) prev ON act.payroll_year = prev.payroll_year+1
	) res3 on res.year +1 = res3.payroll_year 	; -- růst cen a mezd v následujícím roce
